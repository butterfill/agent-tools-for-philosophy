...
awk_filter_bib() {
  ...
  awk -v a_join="$a_join" -v y_join="$y_join" -v d_join="$d_join" -v t_join="$t_join" -v ab_join="$ab_join" -v mode="$mode" -v limit="$limit" '
    ...
    function contains(hay,needle){return index(hay, needle)>0}
    BEGIN{
      split(a_join, A, "\t"); split(y_join, Y, "\t"); split(d_join, D, "\t"); split(t_join, T, "\t"); split(ab_join, AB, "\t");
      want_count=limit+0
    }
    /^\s*@[A-Za-z]+\{[^,]+,/ {
      entry=""; depth=0; started=1
      if (match($0, /^\s*@[A-Za-z]+\{([^,]+),/, m)) key=m[1]; else key=""
    }
    {
      if (started){
        line=$0
        ...
        entry = entry $0 "\n"
        if (depth<=0){
          lower=tolower_str(entry)
          gsub(/[{}]/, "", lower)
          ok=1
          for(i in A){ if (A[i] != "" && !contains(lower, "author") ) { ok=0; break } else if (A[i] != "" && !contains(lower, A[i])) { ok=0; break } }
          if (!ok){ entry=""; started=0; next }
          for(i in Y){ if (Y[i] != "" && !contains(lower, Y[i])) { ok=0; break } }
          if (!ok){ entry=""; started=0; next }
          for(i in D){ if (D[i] != "" && !contains(lower, D[i])) { ok=0; break } }
          if (!ok){ entry=""; started=0; next }
          for(i in T){ if (T[i] != "" && !contains(lower, T[i])) { ok=0; break } }
          if (!ok){ entry=""; started=0; next }
          for(i in AB){ if (AB[i] != "" && !contains(lower, AB[i])) { ok=0; break } }
          if (ok){
            if (mode=="cat") {
              print entry
            } else {
            ...
            found++
            if (found>=want_count && want_count>0){ exit }
          }
          entry=""; started=0
        }
      }
...
keys_from_bib() {
  local mode="$1"
  local out
  out=$(awk_filter_bib "$mode" "$limit" "$(join_tab_lower "${authors[@]:-}")" "$(join_tab_lower "${years[@]:-}")" "$(join_tab_lower "${dois[@]:-}")" "$(join_tab_lower "${titles[@]:-}")" "$(join_tab_lower "${abstracts[@]:-}")")
  printf '%s\n' "$out" | sed '/^\s*$/d'
}

keys_from_sources() {
  local q="$1"
  if [[ -z "$q" ]]; then return 0; fi
  if ! command -v rg-sources >/dev/null 2>&1; then
    echo "find-bib.sh: rg-sources not found but --sources was used" >&2
    exit 2
  fi
  if [[ ! -f "$INDEX_FILE" ]]; then
    echo "find-bib.sh: index file not found: $INDEX_FILE (needed for --sources)" >&2
    exit 2
  fi
  tmpfiles=$(mktemp)
  (
    cd "$PAPERS_DIR"
    rg-sources -l -i "$q" | while IFS= read -r p; do basename -- "$p"; done | sed '/^$/d' | sort -u > "$tmpfiles"
  )
  while IFS= read -r name; do
    jq -r --arg n "$name" 'select(.filename==$n) | .key' "$INDEX_FILE"
  done < "$tmpfiles" | sort -u
  rm -f "$tmpfiles"
}

bib_mode="keys"
[[ "$cat_mode" == true ]] && bib_mode="cat"

keys1=""; keys2=""
keys1=$(keys_from_bib "$bib_mode") || true

if [[ -n "$sources_query" ]]; then
  keys2=$(keys_from_sources "$sources_query") || true
fi

emit_keys() {
  local keys_list="$1"
  if [[ -z "$keys_list" ]]; then return 0; fi
  if $with_md; then
    if ! command -v jq >/dev/null 2>&1; then
      echo "find-bib.sh: jq required for --with-md" >&2
      exit 2
    fi
...
if [[ -z "$result_keys" && -n "$free_text" ]]; then
  ...
  if [[ -n "$sources_query" && -n "$keys2" ]]; then
    if [[ -n "$keys1" ]]; then
      result_keys=$(comm -12 <(printf %s
 "$keys1" | sort -u) <(printf %s
 "$keys2" | sort -u) | head -n "$limit")
    else
      result_keys=$(printf %s
 "$keys2" | head -n "$limit")
    fi
  else
    result_keys=$(printf %s
