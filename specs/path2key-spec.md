...
A tiny CLI to infer and validate a BibTeX key from a file path located in (or related to) your papers directory. It prefers an explicit mapping in the index if available, and otherwise infers the key from the filename pattern. It never returns a key without first validating that the key exists via `cite2bib`.
...
- Guarantee: Do not emit a key unless `cite2bib` can resolve it (exit 0).
...
- Validate any candidate key by calling `cite2bib <candidate>` and only emit a key if that returns exit code 0.
...
1) Index lookup:
   ...
   - If a unique `key` is found, validate it via `cite2bib`. On success, emit it.
...
2) Filename heuristic:
   ...
   - Try validation via `cite2bib` with the colon-inserted candidate.
   ...
   - Optional robustness (for mixed-case legacy keys like `Butterfill:2012fk`):
     - If both attempts fail, perform a case-insensitive discovery pass over `BIB_FILE` to find the real key whose colon-stripped form equals `last_word` (ignoring case). If found, validate the discovered key via `cite2bib` and emit on success.

3) If all attempts fail: exit 1 with a message like `path2key: could not resolve a BibTeX key from: <path>`.

## Environment
- `PAPERS_DIR`: default `$HOME/papers`.
- `INDEX_FILE`: default `$PAPERS_DIR/bibtex-index.jsonl`.
- `BIB_FILE`: indirectly used by `cite2bib` for validation (default there is `$HOME/endnote/phd_biblio.bib`).

## Dependencies
- `jq` (for index lookups).
- `cite2bib` (for key validation; must be on `PATH`).
...
- 2: usage/configuration error (e.g., missing `cite2bib`, invalid `--prefer` value)

## Examples
- `path2key "$HOME/papers/joint action notes/sinigaglia2022_motor.md"` → `sinigaglia:2022_motor`
- `path2key "notes/some paper vesper2012_jumping.md"` → `vesper:2012_jumping`
- `path2key "Butterfill2012fk.md"` → `Butterfill:2012fk` (via heuristic + robustness pass)
- `PAPERS_DIR=~/papers path2key "~/papers/agency/butterfill2019_goals.md"` → `butterfill:2019_goals`
...
- If the index provides multiple entries for the same basename, pick the first but still validate via `cite2bib`.
- If validation fails due to casing mismatches in legacy keys, the optional robustness step uses a case-insensitive scan to discover the actual key, then re-validates with `cite2bib` before emitting.
